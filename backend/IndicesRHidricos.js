const Netcdf_indicesRHidricos = require('./Netcdf/Netcdf_indicesRHidricos.js');
const path = require('path');
const fs = require('fs');

const subirArchivoIndicerh = (req, res) => {
    ...
    if (req.file && authorizationHeader && authorizationHeader.startsWith('Bearer ')) {
        ...
        const nuevoNombre = 'indiceRhidrico_'+sessionId+'.nc';
...
const enviarTiempoIndicerh = (req, res) => {
    const authorizationHeader = req.headers['authorization'];
...
const borrarArchivosIndicerh = (req, res) => {
    const authorizationHeader = req.headers['authorization'];
    if (authorizationHeader && authorizationHeader.startsWith('Bearer ')) {
      const sessionId = authorizationHeader.split(' ')[1];
      // Aquí puedes utilizar el sessionId como lo necesites
      ...
      const rutaArchivoNetcdf = path.join(__dirname, './uploads/',sessionId,'/indiceRhidrico_'+sessionId+'.nc');
      const rutaArchivoJSON = path.join(__dirname, './uploads/',sessionId,'/Lectura_indicesRHidrico_'+sessionId+'.json');
      ...
      try {
        ...
        if (fs.existsSync(rutaArchivoNetcdf)) {
          ...
          archivosEliminados.push('indiceRhidrico_'+sessionId+'.nc');
        }
  
        if (fs.existsSync(rutaArchivoJSON)) {
          fs.unlinkSync(rutaArchivoJSON);
          archivosEliminados.push('Lectura_indicesRHidrico_'+sessionId+'.json');
        }
...
module.exports = {
    subirArchivoIndicerh,
    enviarTiempoIndicerh,
    borrarArchivosIndicerh
};